  ...
  try
    if s:myBufNum == -1
      ...
      if exists('+shellslash')
        "MODIFY: call genutils#OpenWinNoEa("1sp \\\\". escape(s:windowName, ' '))
        call genutils#OpenWinNoEa("abo 1sp \\\\". escape(s:windowName, ' '))
      else
        "MODIFY: call genutils#OpenWinNoEa("1sp \\". escape(s:windowName, ' '))
        call genutils#OpenWinNoEa("abo 1sp \\". escape(s:windowName, ' '))
      endif
      ...
      if winnr == -1
        "MODIFY: call genutils#OpenWinNoEa('1sb '. s:myBufNum)
        call genutils#OpenWinNoEa('abo 1sb '. s:myBufNum)
      else
        let wasVisible = 1
        exec winnr 'wincmd w'
      endif

...
" jwu ADD { 
call s:Set("g:bufExplorerWinDir", "topleft")
call s:Set("g:bufExplorerWinWidth", 30)  
call s:Set("g:bufExplorerWinWidthIncrement", 50)
call s:Set("g:bufExplorerWinHeight", 20)  
call s:Set("g:bufExplorerWinHeightIncrement", 30)
call s:Set("g:bufExplorerUseVertical", 1)
" } jwu ADD end 
...
" jwu ADD { 
" ------------------------------------------------------------------ 
" Desc: Open BufExplorer window 
" ------------------------------------------------------------------ 

function s:exBE_OpenWindow( title ) " <<<
  " open window
  if g:bufExplorerUseVertical
    call exUtility#OpenWindow( a:title, g:bufExplorerWinDir, g:bufExplorerWinWidth, g:bufExplorerUseVertical, 'none', 1, 'none', 'none' )
  else
    call exUtility#OpenWindow( a:title, g:bufExplorerWinDir, g:bufExplorerWinHeight, g:bufExplorerUseVertical, 'none', 1, 'none', 'none' )
  endif
endfunction " >>>
...
function s:exBE_ResizeWindow() " <<<
  if g:bufExplorerUseVertical
    call exUtility#ResizeWindow( g:bufExplorerUseVertical, g:bufExplorerWinWidth, g:bufExplorerWinWidthIncrement )
  else
    call exUtility#ResizeWindow( g:bufExplorerUseVertical, g:bufExplorerWinHeight, g:bufExplorerWinHeightIncrement )
  endif
endfunction " >>>
...
function s:exBE_ToggleWindow( title ) " <<<
  " toggle window
  if g:bufExplorerUseVertical
    call exUtility#ToggleWindow( a:title, g:bufExplorerWinDir, g:bufExplorerWinWidth, g:bufExplorerUseVertical, 'none', 0, 'none', 'none' )
  else
...
function StartBufExplorer(open)
  ...
  let be_winnr = bufwinnr(name)
  if be_winnr == -1
    " open window
    call s:exBE_ToggleWindow(name)
  else
    exe be_winnr . 'wincmd w'
    return
  endif
  " } jwu ADD end 
...
function s:MapKeys()
  ...
  " jwu ADD { 
  " silent exec "nnoremap <buffer> <silent> " . g:ex_keymap_close . " :call <SID>Close()<CR>"
  silent exec "nnoremap <buffer> <silent> " . g:ex_keymap_resize . " :call <SID>exBE_ResizeWindow()<CR>"
  silent exec "nnoremap <buffer> <silent> " . g:ex_keymap_confirm . " :call <SID>SelectBuffer()<CR>"
  nnoremap <buffer> <silent> <2-LeftMouse>   :call <SID>SelectBuffer()<CR>
...
function s:SelectBuffer(...)
  ...
  " jwu ADD { 
  if exists('g:loaded_exutility') && g:loaded_exutility == 1
    let bufname = expand("#"._bufNbr.":p")
